...
module="$(pwd)/module"
rm -rf ${module}
wget -O ${module} "https://raw.githubusercontent.com/SinNombre/Herramientas/main/module/module" &>/dev/null
[[ ! -e ${module} ]] && exit
...
CORALINE="/etc/ADMRufu" && [[ ! -d ${CORALINE} ]] && mkdir ${ADMRufu}
ADM_inst="${CORALINE}/install" && [[ ! -d ${SN_inst} ]] && mkdir ${SN_inst}
tmp="${ADMRufu}/tmp" && [[ ! -d ${tmp} ]] && mkdir ${tmp}
SCPinstal="$HOME/install"

#rm -rf /etc/localtime &>/dev/null
#ln -s /usr/share/zoneinfo/America/Mexico/Tucuman /etc/localtime &>/dev/null
cp -f $0 ${CORALINE}/install.sh
rm $(pwd)/$0 &> /dev/null
if [[ $(which install-LIC) = "" ]]; then
  wget -O /usr/bin/install-LIC 'https://github.com/SinNombre/CARO-LIC/raw/main/install-LIC'; chmod +x /usr/bin/install-LIC &>/dev/null
fi
install-LIC
[[ $? = 1 ]] && exit
...
repo_install(){
  link="https://raw.githubusercontent.com/SinNombre/CORALINE/main/Repositorios/$VERSION_ID.list"
  case $VERSION_ID in
...
verificar_arq(){
  unset ARQ
  case $1 in
    menu|menu_inst.sh|tool_extras.sh|chekup.sh|bashrc)ARQ="${CORALINE}";;
    CORALINE)ARQ="/usr/bin";;
    message.txt)ARQ="${tmp}";;
    *)ARQ="${ADM_inst}";;
  esac
...
post_reboot(){
  echo 'clear; sleep 2; /etc/CORALINE/install.sh --continue' >> /root/.bashrc
...
install_start(){
  ...
  [[ "$opcion" != @(n|N) ]] && source <(curl -sSL "https://raw.githubusercontent.com/SinNombre/CORALINE/main/online/timeZone.sh")
  title "INSTALADOR CORALINE"
  repo_install
...
lisArq="https://raw.githubusercontent.com/SinNombre/CORALINE/main/old"

ver=$(curl -sSL "https://raw.githubusercontent.com/SinNombre/CORALINE/main/vercion")
echo "$ver" > ${CORALINE}/vercion

title -ama '[Proyect by @Rufu99]'
print_center -ama 'INSTALANDO SCRIPT CORALINE'
sleep 2; del 1
...
url='https://github.com/SinNombre/CORALINE/raw/main/Utils'

autoStart="${CORALINE}/bin" && [[ ! -d $autoStart ]] && mkdir $autoStart
varEntorno="${CORALINE}/sbin" && [[ ! -d $varEntorno ]] && mkdir $varEntorno
...
wget --no-cache -O ${CORALINE}/install/udp-custom "$url/udp-custom/udp-custom" &>/dev/null; chmod +x ${ADMRufu}/install/udp-custom
...
if [[ -e $autoStart/autoStart ]]; then
  $autoStart/autoStart -e /etc/CORALINE/autoStart
fi
...
rm -rf /usr/bin/menu
rm -rf /usr/bin/adm
ln -s /usr/bin/CORALINE /usr/bin/menu
ln -s /usr/bin/CORALINE /usr/bin/adm
ln -s /etc/CORALINE/reseller /etc/ADMRufu/tmp/message.txt
sed -i '/Rufu/d' /etc/bash.bashrc
sed -i '/Rufu/d' /root/.bashrc
echo '[[ -e /CORALINE ]] && source /CORALINE' >> /etc/bash.bashrc
update-locale LANG=en_US.UTF-8 LANGUAGE=en
[[ ! $(cat /etc/shells|grep "/bin/false") ]] && echo -e "/bin/false" >> /etc/shells
clear
title "-- CORALINE INSTALADO --"

mv -f ${module} /CORALINE
time_reboot "10"
